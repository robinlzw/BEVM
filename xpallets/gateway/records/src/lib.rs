...
use bevm_primitives::{AddrStr, AssetId};
use xp_assets_registrar::Chain;
use xp_runtime::Memo;
use xpallet_support::try_addr;

pub use self::{
	types::{Withdrawal, WithdrawalRecord, WithdrawalRecordId, WithdrawalState},
	weights::WeightInfo,
};

pub type WithdrawalRecordOf<T> = WithdrawalRecord<
	<T as frame_system::Config>::AccountId,
	<T as xpallet_btc_ledger::Config>::Balance,
	BlockNumberFor<T>,
>;
...
pub mod pallet {
	...
	#[pallet::config]
	pub trait Config: frame_system::Config + xpallet_btc_ledger::Config {
	...
	pub enum Event<T: Config> {
		...
		Deposited(T::AccountId, AssetId, T::Balance),
...
impl<T: Config> Pallet<T> {
	fn ensure_asset_belongs_to_chain(asset_id: AssetId, expected_chain: Chain) -> DispatchResult {
		...
		ensure!(expected_chain == Chain::Bitcoin, Error::<T>::UnexpectedChain);
		Ok(())
	}
...
impl<T: Config> Pallet<T> {
	/// Deposit asset.
	///
	/// NOTE: this function has included deposit_init and deposit_finish (not wait for block
	/// confirm)
	...
	/// when the withdrawal id is passed by the parameter from call(which means the id is from
	/// outside), should pass `Some(chain)` to verify whether the withdrawal is related to this
	/// chain.
	///
	...
	/// ethereum released reserved by trustees submit release request directly, so that we should
	/// check whether the withdrawal belongs to Ethereum Chain, in case release other chain
	/// withdraw.
...
impl<T: Config> Pallet<T> {
	...
	) -> BTreeMap<WithdrawalRecordId, Withdrawal<T::AccountId, T::Balance, BlockNumberFor<T>>> {
		PendingWithdrawals::<T>::iter()
			.map(|(id, record)| {
				(id, Withdrawal::new(record, Self::state_of(id).unwrap_or_default()))
			})
			.collect()
	}
