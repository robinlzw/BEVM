...
///
/// If the (potential) validator failed to meet this requirement, force it to be chilled on new
/// election round.
...
pub struct ValidatorProfile<BlockNumber> {
	...
	///
	/// Declared no desire to be a validator or forced to be chilled due to
	/// `MinimumCandidateThreshold`.
	pub is_chilled: bool,
...
impl MiningDistribution {
	...
	/// Return a tuple (m1, m2) for comparing whether asset_mining_power are reaching the upper
	/// limit.
	///
	/// If m1 >= m2, the asset mining cap has reached, all the reward calculated by the shares go to
	/// the mining assets, but its unit mining power starts to decrease compared to the inital
	/// FixedPower.
