	...
	Balance: Display + FromStr,
{
	...
	#[method(name = "xgatewayrecords_withdrawalList")]
	fn withdrawal_list(
		&self,
		at: Option<BlockHash>,
	) -> RpcResult<BTreeMap<WithdrawalRecordId, RpcWithdrawalRecord<AccountId, Balance, BlockNumber>>>;

	/// Return current withdraw list for a chain(include Applying and Processing withdraw state)
	#[method(name = "xgatewayrecords_withdrawalListByChain")]
	...
	#[method(name = "xgatewayrecords_pendingWithdrawalListByChain")]
...
impl<C, Block, AccountId, Balance, BlockNumber>
	XGatewayRecordsApiServer<<Block as BlockT>::Hash, AccountId, Balance, BlockNumber>
	...
	BlockNumber: Clone + Display + Codec,
{
	...
	) -> RpcResult<BTreeMap<u32, RpcWithdrawalRecord<AccountId, Balance, BlockNumber>>> {
		let api = self.client.runtime_api();
		let at = at.unwrap_or_else(|| self.client.info().best_hash);
		api.withdrawal_list(at)
			.map(|map| map.into_iter().map(|(id, withdrawal)| (id, withdrawal.into())).collect())
