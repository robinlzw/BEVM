...
fn mock_process_tx<T: Config>(tx: Transaction, prev_tx: Option<Transaction>) -> BtcTxState {
	...
	process_tx::<T>(tx, prev_tx, network, min_deposit, current_trustee_pair, previous_trustee_pair)
}
...
fn test_process_tx() {
	...
	ExtBuilder::default().build_and_execute(|| {
		assert_ok!(XAssetsBridge::register(Origin::signed(alice()), 1, H160::from_slice(&ERC20_1)));
		...
		let r =
			mock_process_tx::<Test>(deposit_taproot1.clone(), Some(deposit_taproot1_prev.clone()));
		assert_eq!(r.result, BtcTxResult::Success);
		assert_eq!(
			XGatewayBitcoin::pending_deposits(&deposit_taproot1_input_account.to_vec()),
			vec![BtcDepositCache { txid: deposit_taproot1.hash(), balance: 100000 }]
		);
		...
		let r =
			mock_process_tx::<Test>(deposit_taproot2.clone(), Some(deposit_taproot2_prev.clone()));
		assert_eq!(r.result, BtcTxResult::Success);
...
fn test_push_tx_call() {
	...
	ExtBuilder::default().build_and_execute(|| {
		...
		let info = BtcRelayedTxInfo { block_hash, merkle_proof: proof }.encode();
