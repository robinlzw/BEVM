...
use bevm_runtime::{Block, ExistentialDeposit, RuntimeApi};
...
pub fn run() -> sc_cli::Result<()> {
	...
	match &cli.subcommand {
		...
		Some(Subcommand::Benchmark(_cmd)) => {
			...
			//             "Storage benchmarking can be enabled with `--features
			// runtime-benchmarks`."                 .into(),
			//         ),
			...
			runner.async_run(|config| {
				...
				Ok((
					cmd.run::<Block, ExtendedHostFunctions<
						...
						<BevmExecutorDispatch as NativeExecutionDispatch>::ExtendHostFunctions,
