...
impl<T: Config> ComputeMiningWeight<T::AccountId, BlockNumberFor<T>> for Pallet<T> {
	...
	fn claimer_weight_factors(
		...
		generic_weight_factors::<BalanceOf<T>, BlockNumberFor<T>, _>(claimer_ledger, current_block)
	}
	...
	fn claimee_weight_factors(
		...
		generic_weight_factors::<BalanceOf<T>, BlockNumberFor<T>, _>(claimee_ledger, current_block)
	}
}

type DividendParams<T> =
	(BalanceOf<T>, WeightType, WeightType, <T as frame_system::Config>::AccountId);

impl<T: Config> Pallet<T> {
	/// Returns the tuple of (dividend, source_weight, target_weight) if the nominator claims right
	/// now.
	pub fn calculate_dividend_on_claim(
		...
		let (dividend, source_weight, target_weight) =
			<Self as ComputeMiningWeight<T::AccountId, BlockNumberFor<T>>>::compute_dividend(
				nominator,
				validator,
				block_number,
				reward_pot_balance,
	...
	/// Actually update the nominator vote weight given the new vote weight, block number and amount
	/// delta.
...
impl<T: Config> Claim<T::AccountId> for Pallet<T> {
	...
	fn claim(claimer: &T::AccountId, claimee: &Self::Claimee) -> Result<(), Self::Error> {
		...
		Self::deposit_event(Event::<T>::Claimed(claimer.clone(), claimee.clone(), dividend));
...
/// pub type IdentificationTuple<T> = (<T as crate::Config>::ValidatorId, <T as
/// Config>::FullIdentification); ChainX:
	...
	T::AccountId: UncheckedFrom<T::Hash> + AsRef<[u8]>,
{
	fn reward_pot_account_for(validator: &T::AccountId) -> T::AccountId {
		...
		let registered_at: BlockNumberFor<T> = Validators::<T>::get(validator).registered_at;
