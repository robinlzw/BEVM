...
fn test_normal() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_ok!(XGatewayRecords::process_withdrawals(&numbers, Chain::Bitcoin));
		for i in numbers {
...
fn test_normal2() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_ok!(XGatewayRecords::process_withdrawals(&numbers1, Chain::Bitcoin));
		assert_ok!(XGatewayRecords::process_withdrawals(&numbers2, Chain::Ethereum));

		assert_ok!(XGatewayRecords::finish_withdrawals(&numbers1, Some(Chain::Bitcoin)));
		assert_ok!(XGatewayRecords::finish_withdrawals(&numbers2, Some(Chain::Ethereum)));
...
fn test_withdrawal_force_set_state() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_noop!(XGatewayRecords::cancel_withdrawal(1, &ALICE), XRecordsErr::NotApplyingState);
		...
		assert_eq!(XGatewayRecords::state_of(1), Some(WithdrawalState::Applying));
	})
}
