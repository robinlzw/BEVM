...
fn test_move_trustee_into_little_black_house() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(XGatewayCommon::trustee_sig_record(Chain::Bitcoin, bob()), 10);
		...
		assert_eq!(XGatewayCommon::little_black_house(Chain::Bitcoin), vec![bob()]);
...
fn test_claim_not_native_asset_reward() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(XGatewayCommon::trustee_sig_record(Chain::Bitcoin, charlie()), 1);
		assert_eq!(XGatewayCommon::trustee_sig_record(Chain::Bitcoin, dave()), 0);
