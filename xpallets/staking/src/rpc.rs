...
impl<T: Config> Pallet<T> {
	...
	) -> Vec<ValidatorInfo<T::AccountId, BalanceOf<T>, VoteWeight, BlockNumberFor<T>>> {
		Self::validator_set().map(Self::validator_info_of).collect()
	}

	pub fn validator_info_of(
		who: T::AccountId,
	) -> ValidatorInfo<T::AccountId, BalanceOf<T>, VoteWeight, BlockNumberFor<T>> {
		let profile = Validators::<T>::get(&who);
		let ledger: ValidatorLedger<BalanceOf<T>, VoteWeight, BlockNumberFor<T>> =
	...
	pub fn nomination_details_of(
		who: T::AccountId,
	) -> BTreeMap<T::AccountId, NominatorLedger<BalanceOf<T>, VoteWeight, BlockNumberFor<T>>> {
		Nominations::<T>::iter_prefix(&who)
			.map(|(validator, ledger)| (validator, ledger))
			.collect()
	}

	pub fn nominator_info_of(who: T::AccountId) -> NominatorInfo<BlockNumberFor<T>> {
