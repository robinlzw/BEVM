...
impl DstChainConfig {
	...
	pub fn new(prefix: &[u8], length: u32) -> Self {
		Self { prefix: prefix.to_vec(), length }
	}
}
...
pub fn transfer_named_uncheck(raw_account: &[u8]) -> Option<(Vec<u8>, Vec<u8>)> {
	let name_and_account =
		raw_account.split(|x| *x == b':').map(|d| d.to_vec()).collect::<Vec<_>>();

	if name_and_account.is_empty() || name_and_account.len() != 2 {
		error!("[transfer_named_uncheck] Can't transfer_named_uncheck:{:?}", raw_account);
		return None
	}
	let name = name_and_account[0].clone();
	let account = if name_and_account[1].starts_with(b"0x") {
		hex::decode(&name_and_account[1][2..name_and_account[1].len()]).ok()?
	} else {
...
pub fn from_ss58_check(raw_account: &[u8]) -> Option<AccountId32> {
	...
	#[cfg(not(feature = "ss58check"))]
	{
		...
		if d.len() != len + 3 {
			// Invalid length.
			error!("[from_ss58_check] Bad length, data len:{}, len:{}", d.len(), len);
			return None
		}
