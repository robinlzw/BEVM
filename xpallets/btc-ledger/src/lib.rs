...
use frame_support::{
	...
	traits::{
		...
		tokens::{
			fungible, DepositConsequence, Fortitude, Precision, Preservation, Provenance,
			WithdrawConsequence,
		},
		Currency, EnsureOrigin, ExistenceRequirement, Imbalance, SignedImbalance, TryDrop,
		WithdrawReasons,
	},
	PalletId,
};
...
#[derive(
	Encode, Decode, Default, Clone, Copy, PartialEq, Eq, RuntimeDebug, MaxEncodedLen, TypeInfo,
)]
enum Releases {
	#[default]
	V1_0_0,
	...
	fn transfer(
		...
		Self::try_mutate_account(dest, |to_account, _| -> DispatchResult {
			Self::try_mutate_account(transactor, |from_account, _| -> DispatchResult {
				from_account.free =
					from_account.free.checked_sub(&value).ok_or(Error::<T>::InsufficientBalance)?;
