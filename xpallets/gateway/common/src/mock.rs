...
use crate::{
	...
	traits::{TotalSupply, TrusteeForChain},
	trustees::{
		self,
	...
	for MockBitcoin<T>
{
	...
	> {
		...
		let hot_pks = hot_keys
			.into_iter()
			.map(|k| {
				k.try_into().map_err(|_| xpallet_gateway_bitcoin::Error::<T>::InvalidPublicKey)
			})
		...
		Ok((
			TrusteeSessionInfo {
				trustee_list: trustees.into_iter().zip(vec![1u64; trustee_num]).collect::<Vec<_>>(),
				multi_account: None,
...
impl ExtBuilder {
	pub fn build(self) -> sp_io::TestExternalities {
		...
		GenesisBuild::<Test>::assimilate_storage(
			&xpallet_assets_registrar::GenesisConfig { assets: init_assets },
			&mut storage,
		)
		.unwrap();
		...
		let _ =
			crate::GenesisConfig::<Test> { trustees: trustees() }.assimilate_storage(&mut storage);
...
fn trustees() -> Vec<(Chain, TrusteeInfoConfig, Vec<(AccountId, Vec<u8>, Vec<u8>, Vec<u8>)>)> {
	...
	let btc_config = TrusteeInfoConfig { min_trustee_count: 3, max_trustee_count: 15 };
