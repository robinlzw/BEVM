...
pub fn extract_output_addr(output: &TransactionOutput, network: Network) -> Option<Address> {
	...
	match script_type {
		...
		ScriptType::WitnessV1Taproot => {
			...
			Some(Address { network, kind: address.kind, hash: address.hash })
		},
		_ => None,
	}
}
...
fn test_extract_opreturn_data() {
	...
	// null data script: 6a 30
	// 35524745397a4a79667834367934467948444a65317976394e44725946435446746e6e6d714e445077506a6877753871
	...
	assert_eq!(data, b"5RGE9zJyfx46y4FyHDJe1yv9NDrYFCTFtnnmqNDPwPjhwu8q".to_vec());

	// tx: 003e7e005b172fe0046fd06a83679fbcdc5e3dd64c8ef9295662a463dea486aa
	// null data script: 6a 38
	// 35515a5947565655507370376362714755634873524a555a726e6d547545796836534c48366a6470667346786770524b404c616f63697573
	...
	let data = extract_opreturn_data(&script).unwrap();
	assert_eq!(data, b"5QZYGVVUPsp7cbqGUcHsRJUZrnmTuEyh6SLH6jdpfsFxgpRK@Laocius".to_vec());
}
