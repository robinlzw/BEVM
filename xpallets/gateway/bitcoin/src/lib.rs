...
use bevm_primitives::{AssetId, ReferralId};
...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		/// Trustee create a proposal for a withdrawal list. `tx` is the proposal withdrawal
		/// transaction.
		#[pallet::weight({0})]
		#[pallet::call_index(2)]
		...
		/// Dangerous! remove current withdrawal proposal directly. Please check business logic
		/// before do this operation.
		#[pallet::weight({0})]
		#[pallet::call_index(4)]
		#[transactional]
	...
	pub enum Event<T: Config> {
		...
		/// A list of withdrawal applications were processed successfully. [tx_hash,
		/// withdrawal_ids, total_withdrawn]
		Withdrawn(H256, Vec<u32>, T::Balance),
		/// A new record of unclaimed deposit. [tx_hash, btc_address]
		UnclaimedDeposit(H256, BtcAddress),
		/// A unclaimed deposit record was removed for wasm address. [depositor, deposit_amount,
		/// tx_hash, btc_address]
		...
		DepositedEvm(H256, H160, T::Balance),
		/// A unclaimed deposit record was removed for evm address. [depositor, deposit_amount,
		/// tx_hash, btc_address]
		PendingDepositEvmRemoved(H160, T::Balance, H256, BtcAddress),
		/// An account deposited some token for aptos address. [tx_hash, who, amount]
		DepositedAptos(H256, H256, T::Balance),
		/// A unclaimed deposit record was removed for aptos address. [depositor, deposit_amount,
		/// tx_hash, btc_address]
		PendingDepositAptosRemoved(H256, T::Balance, H256, BtcAddress),
		/// An account deposited some token for named address. [tx_hash, prefix, who, amount]
		DepositedNamed(H256, Vec<u8>, Vec<u8>, T::Balance),
		/// A unclaimed deposit record was removed for named address. [prefix, depositor,
		/// deposit_amount, tx_hash, btc_address]
		PendingDepositNamedRemoved(Vec<u8>, Vec<u8>, T::Balance, H256, BtcAddress),
	}
	...
	impl<T: Config> BuildGenesisConfig for GenesisConfig<T> {
		fn build(&self) {
			...
			let genesis_index = BtcHeaderIndex { hash: *genesis_hash, height: *genesis_height };
			let header_info = BtcHeaderInfo { header: *genesis_header, height: *genesis_height };
	...
	impl<T: Config> ChainT<T::Balance> for Pallet<T> {
		fn asset_id() -> AssetId {
			Default::default()
		}
		fn chain() -> Chain {
		...
		fn withdrawal_limit(
			...
			ensure!(*asset_id == Self::asset_id(), Error::<T>::NotSupportedAsset,);
	...
	impl<T: Config> TotalSupply<T::Balance> for Pallet<T> {
		fn total_supply() -> T::Balance {
			let pending_deposits: T::Balance = PendingDeposits::<T>::iter_values()
				.map(|deposits| deposits.into_iter().map(|deposit| deposit.balance).sum::<u64>())
				.sum::<u64>()
				.saturated_into();

			let asset_supply = xpallet_btc_ledger::Pallet::<T>::total_incoming();
	...
	impl<T: Config> Pallet<T> {
		...
		pub(crate) fn apply_push_header(header: BtcHeader) -> DispatchResult {
			...
			let header_info = BtcHeaderInfo { header, height: prev_info.height + 1 };
			// verify header
			...
			if header_info.height > best_index.height {
				...
				// new best index
				let new_best_index = BtcHeaderIndex { hash, height: header_info.height };
		...
		pub(crate) fn apply_push_transaction(
			...
			// e.g. a tx may be packed in main chain block, and forked chain block, only submit main
			// chain tx could pass the verify.
			...
			// check whether replayed tx has been processed, just process failed and not processed
			// tx;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn verify_bs58_address(data: &[u8]) -> Result<Address, DispatchError> {
			let r = bs58::decode(data).into_vec().map_err(|_| Error::<T>::InvalidBase58)?;
