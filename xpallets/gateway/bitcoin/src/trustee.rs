...
) -> Result<TrusteeSessionInfo<T::AccountId, BlockNumberFor<T>, BtcTrusteeAddrInfo>, DispatchError>
{
...
pub fn check_keys<T: Config>(keys: &[Public]) -> DispatchResult {
	...
	if has_duplicate {
		log!(error, "[generate_new_trustees] Keys contains duplicate pubkey");
		return Err(Error::<T>::DuplicatedKeys.into())
	}
	let has_normal_pubkey = keys.iter().any(|public: &Public| matches!(public, Public::Normal(_)));
...
//const EC_P = Buffer.from('fffffffffffffffffffffffffffffffffffffffffffffffffffffffefffffc2f',
// 'hex')
const EC_P: [u8; 32] = [
...
impl<T: Config> TrusteeForChain<T::AccountId, BlockNumberFor<T>, BtcTrusteeType, BtcTrusteeAddrInfo>
	for Pallet<T>
{
	fn check_trustee_entity(raw_addr: &[u8]) -> Result<BtcTrusteeType, DispatchError> {
		...
		let public: PublicKey = public.try_into().map_err(|_| Error::<T>::InvalidPublicKey)?;
	...
	fn generate_trustee_session_info(
		props: Vec<(T::AccountId, TrusteeIntentionProps<T::AccountId, BtcTrusteeType>)>,
		config: TrusteeInfoConfig,
	) -> Result<
		(
			TrusteeSessionInfo<T::AccountId, BlockNumberFor<T>, BtcTrusteeAddrInfo>,
	...
	> {
		...
		let sig_num = max(
			...
			compute_min_threshold(trustees.len() as u32, MAX_TAPROOT_NODES),
		);
		...
		Ok((
			TrusteeSessionInfo {
				trustee_list: trustees.into_iter().zip(vec![0u64; trustee_num]).collect::<Vec<_>>(),
				multi_account: None,
			...
			ScriptInfo { agg_pubkeys, personal_accounts },
		))
	}
}
...
impl<T: Config> Pallet<T> {
	pub fn ensure_trustee_or_bot(who: &T::AccountId) -> DispatchResult {
		...
		if trustee_session_info.trustee_list.iter().any(|n| &n.0 == who) {
			Ok(())
		} else {
			log!(
				error,
	...
	pub fn apply_create_taproot_withdraw(
		...
		log!(info, "[apply_create_withdraw] Create new withdraw, id_list:{:?}", withdrawal_id_list);
		...
		log!(info, "[apply_create_withdraw] Pass the legality check of withdrawal");

		Self::deposit_event(Event::<T>::WithdrawalProposalCreated(who, withdrawal_id_list));
...
pub(crate) fn create_multi_address<T: Config>(
	...
	if sum > 15 {
		log!(error, "Bitcoin's multisig can't more than 15, current:{}", sum);
		return None
	}
	...
	let redeem_script =
		build.push_opcode(opcode).push_opcode(Opcode::OP_CHECKMULTISIG).into_script();
