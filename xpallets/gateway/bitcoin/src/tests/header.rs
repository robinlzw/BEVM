...
use crate::{
	mock::{
		alice, generate_blocks_478557_478563, generate_blocks_63290_63310, ExtBuilder,
		XGatewayBitcoin, XGatewayBitcoinErr,
	},
...
fn test_genesis() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(index, BtcHeaderIndex { hash: header.hash(), height: 63290 });
	})
}
...
fn test_insert_forked_headers_from_genesis_height() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			// add a forked block exceed confirmed height, this forked block would mark as
			// AncientFork, but now this forked block is less then best, so do not do confirmed
			// check and on the other hand, confirmed block for this forked block is before current
			// normal
