...
pub fn work_required<T: Config>(
	...
	let parent_header: BtcHeader =
		Pallet::<T>::headers(parent_hash).expect("pre header must exist here").header;
...
fn work_required_retarget<T: Config>(
	...
	retarget *= U256::from(retarget_timespan(retarget_timestamp, last_timestamp, params));
	...
	RequiredWork::Value(if retarget > maximum { params.max_bits() } else { retarget.into() })
}
...
fn retarget_timespan(retarget_timestamp: u32, last_timestamp: u32, params: &BtcParams) -> u32 {
	...
	range_constrain(timespan, i64::from(params.min_timespan()), i64::from(params.max_timespan()))
		as u32
}
