...
use std::{collections::btree_map::BTreeMap, fmt::Display, str::FromStr, sync::Arc};
...
use xp_rpc::{runtime_error_into_rpc_err, RpcBalance, RpcVoteWeight};
	...
	VoteWeight: Display + FromStr,
{
	...
	#[method(name = "xstaking_getValidators")]
	fn validators(
		&self,
		at: Option<BlockHash>,
	) -> RpcResult<
	...
	#[method(name = "xstaking_getValidatorByAccount")]
	...
	#[method(name = "xstaking_getDividendByAccount")]
	fn staking_dividend_of(
		&self,
		who: AccountId,
		at: Option<BlockHash>,
	) -> RpcResult<BTreeMap<AccountId, RpcBalance<Balance>>>;

	/// Get the nomination details given the staker AccountId.
	#[method(name = "xstaking_getNominationByAccount")]
	...
	#[method(name = "xstaking_getNominatorByAccount")]
	fn nominator_info_of(
		&self,
		who: AccountId,
		at: Option<BlockHash>,
	) -> RpcResult<NominatorInfo<BlockNumber>>;
}
...
impl<C, Block, AccountId, Balance, VoteWeight, BlockNumber>
	XStakingApiServer<<Block as BlockT>::Hash, AccountId, Balance, VoteWeight, BlockNumber>
	...
	BlockNumber: Codec,
{
	...
	fn nominator_info_of(
		...
		let api = self.client.runtime_api();
		let at = at.unwrap_or_else(|| self.client.info().best_hash);
		api.nominator_info_of(at, who).map_err(runtime_error_into_rpc_err)
	}
}
