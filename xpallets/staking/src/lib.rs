...
//! ## Terminology
//!
//! - Validator nickname: The nickname, a.k.a, Validator name in ChainX 1.0, is exclusively used as
//!   the ReferralId for getting some reward(10% of the total dividend) in Asset Mining because the
//!   depositor marks this validator as its referral when doing the deposit. Validator nickname and
//!   ReferralId is interchangeable in various scenarios.
...
use bevm_primitives::ReferralId;
...
pub use self::{
	impls::{IdentificationTuple, SimpleValidatorRewardPotAccountDeterminer},
	rpc::*,
	types::*,
	weights::WeightInfo,
};
...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn set_bonding_duration(
			origin: OriginFor<T>,
			#[pallet::compact] new: BlockNumberFor<T>,
		) -> DispatchResult {
			ensure_root(origin)?;
		...
		pub fn set_immortals(origin: OriginFor<T>, new: Vec<T::AccountId>) -> DispatchResult {
			ensure_root(origin)?;
			ensure!(new.iter().all(|x| Self::is_validator(x)), Error::<T>::NotValidator);
			if new.is_empty() {
				Immortals::<T>::kill()
			} else {
				Immortals::<T>::put(new);
	...
	pub enum Event<T: Config> {
		...
		/// Issue new balance to validator and pot. [validator, reward_amount, validator_pot,
		/// reward_amount]
		...
		/// A nominator switched the vote from one validator to another. [nominator, from, to,
		/// amount]
		...
		/// Offenders were forcibly to be chilled due to insufficient reward pot balance.
		/// [session_index, chilled_validators]
	...
	pub enum Error<T> {
		...
		/// The validator can not (forcedly) be chilled due to the limit of minimal validators
		/// count.
		...
		NoMoreRebond,
		/// An account can only rebond the balance that is no more than what it has bonded to the
		/// validator.
		InvalidRebondBalance,
		/// Can not rebond the validator self-bonded votes as it has a much longer bonding
		/// duration.
		RebondSelfBondedNotAllowed,
		/// An account can only unbond the balance that is no more than what it has bonded to the
		/// validator.
	...
	#[pallet::type_value]
	pub fn DefaultForBondingDuration<T: Config>() -> BlockNumberFor<T> {
		BlockNumberFor::<T>::saturated_from::<u64>(DEFAULT_BONDING_DURATION)
	}
	...
	pub type BondingDuration<T: Config> =
		StorageValue<_, BlockNumberFor<T>, ValueQuery, DefaultForBondingDuration<T>>;

	#[pallet::type_value]
	pub fn DefaultForValidatorBondingDuration<T: Config>() -> BlockNumberFor<T> {
		BlockNumberFor::<T>::saturated_from::<u64>(DEFAULT_VALIDATOR_BONDING_DURATION)
	}
	...
	pub type ValidatorBondingDuration<T: Config> =
		StorageValue<_, BlockNumberFor<T>, ValueQuery, DefaultForValidatorBondingDuration<T>>;
	...
	pub type Validators<T: Config> =
		StorageMap<_, Twox64Concat, T::AccountId, ValidatorProfile<BlockNumberFor<T>>, ValueQuery>;
	...
	pub type ValidatorLedgers<T: Config> = StorageMap<
		...
		Twox64Concat,
		T::AccountId,
		ValidatorLedger<BalanceOf<T>, VoteWeight, BlockNumberFor<T>>,
		ValueQuery,
	>;
	...
	pub type Nominations<T: Config> = StorageDoubleMap<
		...
		Twox64Concat,
		T::AccountId,
		NominatorLedger<BalanceOf<T>, VoteWeight, BlockNumberFor<T>>,
		ValueQuery,
	>;
	...
	pub type LastRebondOf<T: Config> = StorageMap<_, Twox64Concat, T::AccountId, BlockNumberFor<T>>;
	...
	pub struct GenesisConfig<T: Config> {
		...
		pub bonding_duration: BlockNumberFor<T>,
		pub validator_bonding_duration: BlockNumberFor<T>,
...
impl<T: Config> Pallet<T> {
	...
	pub fn active_candidates() -> Vec<T::AccountId> {
		...
		candidates.sort_by(|(b1, _), (b2, _)| b2.cmp(b1));
	...
	fn unbonded_chunks_of(
		...
		target: &T::AccountId,
	) -> Vec<Unbonded<BalanceOf<T>, BlockNumberFor<T>>> {
	...
	fn check_referral_id(referral_id: &[u8]) -> Result<(), Error<T>> {
		...
		ensure!(xp_runtime::xss_check(referral_id).is_ok(), Error::<T>::XssCheckFailed);
		ensure!(Self::validator_for(referral_id).is_none(), Error::<T>::OccupiedReferralIdentity);
		Ok(())
	}
	...
	pub(crate) fn bond_reserve(who: &T::AccountId, value: BalanceOf<T>) {
		Locks::<T>::mutate(who, |locks| {
			*locks.entry(LockedType::Bonded).or_default() += value;
			...
			let staking_locked = locks.values().fold(Zero::zero(), |acc: BalanceOf<T>, x| acc + *x);
	...
	fn total_locked_of(who: &T::AccountId) -> BalanceOf<T> {
		Self::locks(who).values().fold(Zero::zero(), |acc: BalanceOf<T>, x| acc + *x)
	}
	...
	fn update_vote_weight(
		...
		let source_weight =
			<Self as ComputeMiningWeight<T::AccountId, BlockNumberFor<T>>>::settle_claimer_weight(
				source,
				target,
				current_block,
			);
		...
		let target_weight =
			<Self as ComputeMiningWeight<T::AccountId, BlockNumberFor<T>>>::settle_claimee_weight(
				target,
				current_block,
			);
	...
	fn apply_register(who: &T::AccountId, referral_id: ReferralId) {
		...
		Validators::<T>::insert(
			who,
			ValidatorProfile { registered_at: current_block, referral_id, ..Default::default() },
		);
	}
	...
	fn apply_bond(
		...
		Self::deposit_event(Event::<T>::Bonded(nominator.clone(), nominee.clone(), value));
		Ok(())
	}
	...
	fn apply_rebond(
		...
		Self::deposit_event(Event::<T>::Rebonded(who.clone(), from.clone(), to.clone(), value));
	}
	...
	fn mutate_unbonded_chunks(
		...
		Nominations::<T>::mutate(who, target, |nominator| {
			if let Some(idx) =
				nominator.unbonded_chunks.iter().position(|x| x.locked_until == locked_until)
			{
				nominator.unbonded_chunks[idx].value += value;
			} else {
				nominator.unbonded_chunks.push(Unbonded { value, locked_until });
			}
		});
	}

	fn bonding_duration_for(who: &T::AccountId, target: &T::AccountId) -> BlockNumberFor<T> {
	...
	fn apply_unlock_unbonded_withdrawal(who: &T::AccountId, value: BalanceOf<T>) {
		...
		Locks::<T>::mutate(who, |locks| {
			...
			if old_value == value {
				...
				locks.insert(LockedType::BondedWithdrawal, old_value.saturating_sub(value));
			}
		});
	}
}
