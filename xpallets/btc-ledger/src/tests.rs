...
use sp_runtime::{traits::BadOrigin, ArithmeticError, BuildStorage};
...
fn reward_should_work() {
	new_test_ext().execute_with(|| {
		...
		System::assert_last_event(RuntimeEvent::BtcLedger(crate::Event::Deposit {
			who: ALICE.into(),
			amount: 10,
		}));
...
fn balance_works() {
	new_test_ext().execute_with(|| {
		...
		System::assert_has_event(RuntimeEvent::BtcLedger(crate::Event::Deposit {
			who: ALICE.into(),
			amount: 30,
		}));
...
fn withdrawing_balance_should_work() {
	new_test_ext().execute_with(|| {
		...
		System::assert_last_event(RuntimeEvent::BtcLedger(crate::Event::Withdraw {
			who: BOB.into(),
			amount: 20,
		}));
...
fn transfer_all_free_succeed() {
	new_test_ext().execute_with(|| {
		assert_ok!(BtcLedger::set_balance(RuntimeOrigin::root(), ALICE.into(), 100));
...
fn transfer_all_works() {
	new_test_ext().execute_with(|| {
		// setup
		assert_ok!(BtcLedger::set_balance(RuntimeOrigin::root(), ALICE.into(), 200));
		assert_ok!(BtcLedger::set_balance(RuntimeOrigin::root(), BOB.into(), 0));
