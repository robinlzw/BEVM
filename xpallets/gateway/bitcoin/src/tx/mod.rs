...
use xp_assets_registrar::ChainT;
...
fn deposit_wasm<T: Config>(txid: H256, who: &T::AccountId, balance: u64) -> DispatchResult {
	...
	let value: T::Balance = balance.saturated_into();
...
fn deposit_evm<T: Config>(_txid: H256, _who: &H160, _balance: u64) -> DispatchResult {
	// let id: AssetId = <Pallet<T> as ChainT<_>>::ASSET_ID;
	//
...
fn withdraw<T: Config>(tx: Transaction) -> BtcTxResult {
	if let Some(proposal) = WithdrawalProposal::<T>::take() {
		...
		Pallet::<T>::deposit_event(Event::<T>::WithdrawalFatalErr(tx.hash(), Default::default()));
