...
use xp_rpc::{
	...
	trustee_inexistent_rpc_err, RpcBalance,
};

use xpallet_gateway_common_rpc_runtime_api::{
	trustees::bitcoin::{BtcTrusteeIntentionProps, BtcTrusteeSessionInfo},
	...
	Balance: Display + FromStr,
{
	...
	#[method(name = "xgatewaycommon_boundAddrs")]
	fn bound_addrs(
		&self,
		who: AccountId,
		at: Option<BlockHash>,
	) -> RpcResult<BTreeMap<Chain, Vec<String>>>;

	/// Get withdrawal limit(minimal_withdrawal&fee) for an AssetId
	#[method(name = "xgatewaycommon_withdrawalLimit")]
	fn withdrawal_limit(
		&self,
		asset_id: AssetId,
		at: Option<BlockHash>,
	) -> RpcResult<WithdrawalLimit<RpcBalance<Balance>>>;

	/// Get a list of withdrawals with withdrawal fee information.
	#[method(name = "xgatewaycommon_withdrawalListWithFeeInfo")]
	...
	/// Use the params to verify whether the withdrawal apply is valid. Notice those params is same
	/// as the params for call `XGatewayCommon::withdraw(...)`, including checking address is valid
	/// or something else. Front-end should use this rpc to check params first, than could create
	/// the extrinsic.
	#[method(name = "xgatewaycommon_verifyWithdrawal")]
	...
	#[method(name = "xgatewaycommon_trusteeMultisigs")]
	fn multisigs(&self, at: Option<BlockHash>) -> RpcResult<BTreeMap<Chain, AccountId>>;

	/// Return bitcoin trustee registered property info for an account(e.g. registered hot/cold
	/// address)
	#[method(name = "xgatewaycommon_bitcoinTrusteeProperties")]
	fn btc_trustee_properties(
		&self,
		who: AccountId,
		at: Option<BlockHash>,
	) -> RpcResult<BtcTrusteeIntentionProps<AccountId>>;

	/// Return bitcoin trustee for current session(e.g. trustee hot/cold address and else)
	#[method(name = "xgatewaycommon_bitcoinTrusteeSessionInfo")]
	fn btc_trustee_session_info(
		&self,
		session_number: i32,
		at: Option<BlockHash>,
	) -> RpcResult<BtcTrusteeSessionInfo<AccountId, BlockNumber>>;

	/// Try to generate bitcoin trustee info for a list of candidates. (this api is used to check
	/// the trustee info which would be generated by those candidates)
	#[method(name = "xgatewaycommon_bitcoinGenerateTrusteeSessionInfo")]
	...
	BlockNumber: Codec + Send + Sync + 'static,
{
	...
	) -> RpcResult<GenericTrusteeIntentionProps<AccountId>> {
	...
	fn generic_trustee_session_info(
		...
		session_number: i32,
		at: Option<<Block as BlockT>::Hash>,
	) -> RpcResult<GenericTrusteeSessionInfo<AccountId, BlockNumber>> {
		let api = self.client.runtime_api();
		let at = at.unwrap_or_else(|| self.client.info().best_hash);
	...
	fn generate_generic_trustee_session_info(
		...
		candidates: Vec<AccountId>,
		at: Option<<Block as BlockT>::Hash>,
	) -> RpcResult<(GenericTrusteeSessionInfo<AccountId, BlockNumber>, ScriptInfo<AccountId>)> {
		let api = self.client.runtime_api();
		let at = at.unwrap_or_else(|| self.client.info().best_hash);
...
impl<C, Block, AccountId, Balance, BlockNumber>
	XGatewayCommonApiServer<<Block as BlockT>::Hash, AccountId, Balance, BlockNumber>
	...
	BlockNumber: Codec + Send + Sync + 'static,
{
	fn bound_addrs(
		...
		let result = api.bound_addrs(at, who).map_err(runtime_error_into_rpc_err)?;
	...
	fn multisigs(
		&self,
		at: Option<<Block as BlockT>::Hash>,
	) -> RpcResult<BTreeMap<Chain, AccountId>> {
		let api = self.client.runtime_api();
		let at = at.unwrap_or_else(||
            // If the block hash is not supplied assume the best block.
            self.client.info().best_hash);

		let result = api.trustee_multisigs(at).map_err(runtime_error_into_rpc_err)?;
