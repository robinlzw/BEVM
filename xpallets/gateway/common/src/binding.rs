...
use bevm_primitives::{AssetId, ChainAddress, ReferralId};
...
impl<T: Config> ReferralBinding<T::AccountId> for Pallet<T> {
	fn update_binding(asset_id: &AssetId, who: &T::AccountId, referral_name: Option<ReferralId>) {
		if *asset_id != AssetId::default() {
			error!(
				...
				"[update_referral_binding] Unexpected asset_id:{:?}",
				asset_id
			);
			return
		}
	...
	fn referral(asset_id: &AssetId, who: &T::AccountId) -> Option<T::AccountId> {
		...
		Self::referral_binding_of(who, Chain::Bitcoin)
	}
}
		...
		Address: Into<Vec<u8>>,
	{
		...
		if let Some(accountid) = AddressBindingOf::<T>::get(chain, &address) {
			if accountid != who {
				...
				// old accountid is not equal to new accountid, means should change this addr bind
				// to new account remove this addr for old accounid's CrossChainBindOf
