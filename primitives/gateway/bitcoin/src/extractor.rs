...
impl AccountExtractor<AccountId32, ReferralId> for OpReturnExtractor {
	fn extract_account(data: &[u8]) -> Option<(OpReturnAccount<AccountId32>, Option<ReferralId>)> {
		let account_and_referral =
			data.split(|x| *x == b'@').map(|d| d.to_vec()).collect::<Vec<_>>();

		if account_and_referral.is_empty() {
			error!("[extract_account] Can't extract account from data:{:?}", hex::encode(data));
			return None
		}
		...
		debug!("[extract_account] account:{:?}, referral:{:?}", account, referral);
...
fn test_opreturn_extractor() {
...

	{
		...
		assert_eq!(result, Some((OpReturnAccount::Wasm(addr.unchecked_into()), None)));
		...
		assert_eq!(
			result,
			Some((OpReturnAccount::Wasm(addr.unchecked_into()), Some(b"referral1".to_vec())))
		);
		...
		assert_eq!(result, Some((OpReturnAccount::Evm(evm_addr), Some(b"referral1".to_vec()))));
		...
		assert_eq!(result, Some((OpReturnAccount::Aptos(aptos_addr), Some(b"referral1".to_vec()))));
		...
		assert_eq!(
			result,
			Some((OpReturnAccount::Named(name.clone(), addr.clone()), Some(b"referral1".to_vec())))
		);

		let result = OpReturnExtractor::extract_account(
			"sui:1dcba11f07596152cf96a9bd358b675d5d5f9506@referral1".as_bytes(),
		);
		assert_eq!(result, Some((OpReturnAccount::Named(name, addr), Some(b"referral1".to_vec()))));
	}
	{
