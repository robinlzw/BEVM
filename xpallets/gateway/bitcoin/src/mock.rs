...
impl ExtBuilder {
	...
	pub fn build(self) -> sp_io::TestExternalities {
		...
		xpallet_assets_bridge::GenesisConfig::<Test> { admin_key: Some(alice()) }
			.assimilate_storage(&mut storage)
			.unwrap();
		...
		let genesis_trustees = info
			...
			.find_map(|(chain, _, trustee_params)| {
				if *chain == Chain::Bitcoin {
					Some(trustee_params.iter().map(|i| (i.0).clone()).collect::<Vec<_>>())
				} else {
					None
				}
			})
...
fn trustees_info() -> Vec<(Chain, TrusteeInfoConfig, Vec<(AccountId, Vec<u8>, Vec<u8>, Vec<u8>)>)> {
