...
const RUNTIME_TRUSTEE_ERROR: i32 = RUNTIME_ERROR + 100;

/// Decode the generic trustee info failed.
pub const RUNTIME_TRUSTEE_DECODE_ERROR: i32 = RUNTIME_TRUSTEE_ERROR + 1;

/// The trustees are inexistent.
pub const RUNTIME_TRUSTEE_INEXISTENT_ERROR: i32 = RUNTIME_TRUSTEE_ERROR + 2;

/// The transaction was not decodable.
pub const DECODE_ERROR: i32 = 10000;

/// The bytes failed to be decoded as hex.
pub const DECODE_HEX_ERROR: i32 = DECODE_ERROR + 1;
...
pub fn trustee_decode_error_into_rpc_err(err: impl Debug) -> Error {
	Error::Call(CallError::Custom(ErrorObject::owned(
		RUNTIME_TRUSTEE_DECODE_ERROR,
		"Can not decode generic trustee session info",
		Some(format!("{:?}", err)),
	)))
}
...
pub fn trustee_inexistent_rpc_err() -> Error {
	Error::Call(CallError::Custom(ErrorObject::owned(
		RUNTIME_TRUSTEE_INEXISTENT_ERROR,
...
pub mod serde_num_str {
		...
		T: FromStr,
	{
		...
		data.parse::<T>().map_err(|_| de::Error::custom("Parse from string failed"))
	}
}
...
pub mod serde_hex {
	...
	{
		let data = String::deserialize(deserializer)?;
		let data = if let Some(stripped) = data.strip_prefix("0x") { stripped } else { &data[..] };
