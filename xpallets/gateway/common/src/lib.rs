...
use frame_support::{
	pallet_prelude::{ensure, DispatchError, DispatchResult},
...
/// BEVM primitives
use bevm_primitives::{AddrStr, AssetId, ChainAddress, Text};
use xp_assets_registrar::{Chain, ChainT, WithdrawalLimit};
...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		///
		/// NOTE: `ext` is for the compatibility purpose, e.g., EOS requires a memo when doing the
		/// transfer.
		#[pallet::weight({0})]
		#[pallet::call_index(0)]
		#[transactional]
		...
		///
		/// This is to allow for timely replacement in the event of a problem with a particular
		/// trustee. The trustee will be moved into the small black room.
		///
		/// This is called by the trustee admin and root.
		/// # <weight>
		/// Since this is a root call and will go into trustee election, we assume full block for
		/// now. # </weight>
		#[pallet::weight({0})]
		#[pallet::call_index(5)]
		#[transactional]
		...
		pub fn claim_trustee_reward(
			...
			if session_num == Self::trustee_session_info_len(chain) {
				<T as pallet::Config>::CouncilOrigin::ensure_origin(origin)?;
	...
	pub enum Event<T: Config> {
		...
		SetTrusteeProps(T::AccountId, Chain, GenericTrusteeIntentionProps<T::AccountId>),
		...
		TrusteeSetChanged(
			Chain,
			u32,
			GenericTrusteeSessionInfo<T::AccountId, BlockNumberFor<T>>,
			u32,
		),
		/// Treasury transfer to trustee. [source, target, chain, session_number, reward_total]
		TransferTrusteeReward(T::AccountId, T::AccountId, Chain, u32, T::Balance),
		/// Asset reward to trustee multi_account. [target, asset_id, reward_total]
		TransferAssetReward(T::AccountId, AssetId, T::Balance),
		/// The native asset of trustee multi_account is assigned. [multi_account, session_number,
		/// total_reward]
		AllocNativeReward(T::AccountId, u32, T::Balance),
		/// The not native asset of trustee multi_account is assigned. [multi_account,
		/// session_number, asset_id, total_reward]
		AllocNotNativeReward(T::AccountId, u32, AssetId, T::Balance),
	}
	...
	pub(crate) type PreTotalSupply<T: Config> =
		StorageDoubleMap<_, Twox64Concat, Chain, Twox64Concat, u32, T::Balance, ValueQuery>;

	#[pallet::genesis_config]
	pub struct GenesisConfig<T: Config> {
		pub trustees: Vec<(Chain, TrusteeInfoConfig, Vec<(T::AccountId, Text, Vec<u8>, Vec<u8>)>)>,
	}

	#[cfg(feature = "std")]
	impl<T: Config> Default for GenesisConfig<T> {
		fn default() -> Self {
			Self { trustees: Default::default() }
		}
	}
...
impl<T: Config> Pallet<T> {
	pub fn verify_withdrawal(
		...
		ensure!(asset_id == AssetId::default(), Error::<T>::NotSupportedAsset);
		...
		ensure!(value >= limit.minimal_withdrawal, Error::<T>::InvalidWithdrawal);
		Ok(())
	}
}
...
impl<T: Config> Pallet<T> {
	pub fn setup_trustee_impl(
		...
		let proxy_account =
			if let Some(addr) = proxy_account { Some(addr) } else { Some(who.clone()) };
	...
	pub fn set_trustee_proxy_impl(who: &T::AccountId, proxy_account: T::AccountId, chain: Chain) {
		...
		Self::deposit_event(Event::<T>::SetTrusteeProxy(who.clone(), chain, proxy_account));
	}
...
impl<T: Config> Pallet<T> {
	...
	pub fn do_trustee_election(chain: Chain) -> DispatchResult {
		ensure!(!Self::trustee_transition_status(chain), Error::<T>::LastTransitionNotCompleted);
		...
		ensure!(new_trustee_pool.len() >= desired_members, Error::<T>::TrusteeMembersNotEnough);
		...
		ensure!(!incoming.is_empty() || !outgoing.is_empty(), Error::<T>::TrusteeMembersNotEnough);
	...
	pub fn try_generate_session_info(
		...
		(GenericTrusteeSessionInfo<T::AccountId, BlockNumberFor<T>>, ScriptInfo<T::AccountId>),
		DispatchError,
	> {
	...
	fn alter_trustee_session(
		...
		// Remove the information of the previous aggregate public key，Withdrawal is prohibited at
		// this time.
		let _ = AggPubkeyInfo::<T>::clear(u32::MAX, None);
	...
	fn transition_trustee_session_impl(
		...
		let session_number = Self::trustee_session_info_len(chain).checked_add(1).unwrap_or(0u32);
	...
	fn cancel_trustee_transition_impl(chain: Chain) -> DispatchResult {
		...
		let trustees =
			trustee_info.0.trustee_list.clone().into_iter().unzip::<_, _, _, Vec<u64>>().0;
	...
	pub fn generate_multisig_addr(
		chain: Chain,
		session_info: &GenericTrusteeSessionInfo<T::AccountId, BlockNumberFor<T>>,
...
impl<T: Config> Pallet<T> {
	...
	//     reward: Balance,
	//     trustee_info: &TrusteeSessionInfo<T::AccountId, BlockNumberFor<T>, BtcTrusteeAddrInfo>,
	...
	//             error!(
	//                 target: "runtime::gateway::common",
	//                 "[apply_claim_trustee_reward] error {:?}, sum_balance:{:?},
	// reward_info:{:?}.",                 e, total_reward, reward_info.clone()
	//             );
	//             e
	//         })?;
	//     }
	...
	//             error!(
	//                 target: "runtime::gateway::common",
	//                 "[apply_claim_trustee_reward] error {:?}, sum_balance:{:?}, asset_id:
	// {:?},reward_info:{:?}.",                 e, total_reward, asset_id, reward_info.clone()
	//             );
	...
	pub fn apply_claim_trustee_reward(_session_num: u32) -> DispatchResult {
		// TODO: fix me
		// let session_info = T::BitcoinTrusteeSessionProvider::trustee_session(session_num)?;
		...
		// }
		// // alloc btc reward
		// match Self::alloc_not_native_reward(&multi_account, BTC, &session_info) {
		//     Ok(total_btc_reward) => {
...
impl<T: Config> Pallet<T> {
	pub fn withdrawal_limit(
		...
		T::Bitcoin::withdrawal_limit(asset_id)
	}
	...
	pub fn withdrawal_list_with_fee_info(
		...
		BTreeMap<
			WithdrawalRecordId,
			(Withdrawal<T::AccountId, T::Balance, BlockNumberFor<T>>, WithdrawalLimit<T::Balance>),
		>,
		DispatchError,
	> {
		let limit = Self::withdrawal_limit(asset_id)?;
