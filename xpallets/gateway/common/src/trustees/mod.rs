...
impl<T: Config, TrusteeAddress: BytesLike + ChainProvider>
	TrusteeSession<T::AccountId, BlockNumberFor<T>, TrusteeAddress>
	for TrusteeSessionManager<T, TrusteeAddress>
{
	fn trustee_session(
		number: u32,
	) -> Result<TrusteeSessionInfo<T::AccountId, BlockNumberFor<T>, TrusteeAddress>, DispatchError>
	{
		...
		let info = TrusteeSessionInfo::<T::AccountId, BlockNumberFor<T>, TrusteeAddress>::try_from(
			generic_info,
		)
...
impl<T: Config> TrusteeInfoUpdate for Pallet<T> {
	fn update_transition_status(chain: Chain, status: bool, _: Option<u64>) {
		...
		if chain == Chain::Bitcoin && Self::trustee_transition_status(chain) && !status {
			...
			let _ = TrusteeSigRecord::<T>::clear_prefix(chain, u32::MAX, None);
		}
