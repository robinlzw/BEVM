...
fn assert_bonded_locks(who: AccountId, value: Balance) {
	assert_eq!(*<Locks<Test>>::get(who).entry(LockedType::Bonded).or_default(), value);
}

fn assert_bonded_withdrawal_locks(who: AccountId, value: Balance) {
	assert_eq!(*<Locks<Test>>::get(who).entry(LockedType::BondedWithdrawal).or_default(), value);
}
...
fn cannot_force_chill_should_work() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_err!(XStaking::chill(Origin::signed(1)), <Error<Test>>::TooFewActiveValidators);
...
fn total_staking_locked_no_more_than_free_balance_should_work() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(Locks::<Test>::get(&1), vec![(LockedType::Bonded, 90)].into_iter().collect());
		assert_eq!(
			frame_system::Account::<Test>::get(&1).data,
			pallet_balances::AccountData {
				free: 100,
	...
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(
			...
			NominatorLedger {
				...
				unbonded_chunks: vec![Unbonded { value: 5, locked_until: 50 * 12 * 24 * 3 + 3 }],
			}
		);
	});
}
...
fn withdraw_unbond_should_work() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(
			...
			vec![Unbonded { value: unbond_value, locked_until: DEFAULT_BONDING_DURATION + 3 }]
		);

		t_system_block_number_inc(DEFAULT_BONDING_DURATION);
		assert_err!(t_withdraw_unbonded(1, 2, 0), Error::<Test>::UnbondedWithdrawalNotYetDue);

		t_system_block_number_inc(1);

		let before_withdraw_unbonded = Balances::usable_balance(&1);
		assert_ok!(t_withdraw_unbonded(1, 2, 0));
		assert_eq!(Balances::usable_balance(&1), before_withdraw_unbonded + unbond_value);
		...
		assert_eq!(
			frame_system::Account::<Test>::get(&1).data,
			pallet_balances::AccountData { free: 100, reserved: 0, misc_frozen: 0, fee_frozen: 0 }
		);
	});
}
...
fn regular_staking_should_work() {
	ExtBuilder::default().build_and_execute(|| {
		...
		//
		// [new_era]current_era:1, start_session_index:3, maybe_new_validators:Some([4, 3, 2, 1, 8,
		// 7]) Session Validators: [4, 3, 2, 1]
		//
...
fn staking_reward_should_work() {
	ExtBuilder::default().build_and_execute(|| {
		...
		let test_validator_reward = |validator: AccountId,
...
fn t_reward_pot_balance(validator: AccountId) -> Balance {
	XStaking::free_balance(&DummyStakingRewardPotAccountDeterminer::reward_pot_account_for(
		&validator,
	))
}
...
fn staker_reward_should_work() {
	ExtBuilder::default().build_and_execute(|| {
		...
		t_start_session(2);
		assert_eq!(t_reward_pot_balance(4), 576_000_000 + 609_230_769 - 121_846_153);
		assert_eq!(
...
fn balances_reserve_should_work() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(XStaking::locks(&who), vec![(LockedType::Bonded, 6)].into_iter().collect());
		assert_eq!(
			frame_system::Account::<Test>::get(&who).data,
			pallet_balances::AccountData { free: 10, reserved: 0, misc_frozen: 6, fee_frozen: 6 }
		);
		assert_err!(
		...
		assert_eq!(XStaking::locks(&who), vec![(LockedType::Bonded, 6 + 2)].into_iter().collect());
		assert_eq!(
			frame_system::Account::<Test>::get(&who).data,
			pallet_balances::AccountData { free: 10, reserved: 0, misc_frozen: 8, fee_frozen: 8 }
		);
		...
		assert_eq!(
			frame_system::Account::<Test>::get(&who).data,
			pallet_balances::AccountData { free: 10, reserved: 0, misc_frozen: 11, fee_frozen: 11 }
		);
...
fn referral_id_should_work() {
	ExtBuilder::default().build_and_execute(|| {
		assert_ok!(XStaking::register(Origin::signed(111), b"referral1".to_vec(), 0));
		assert_err!(
			XStaking::register(Origin::signed(112), b"referral1".to_vec(), 0),
			Error::<Test>::OccupiedReferralIdentity
		);

		assert_ok!(XStaking::register(Origin::signed(112), b"referral2".to_vec(), 0));
	});
}
...
fn migration_session_offset_should_work() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(XStaking::this_session_reward(), (INITIAL_REWARD / 2) as u128);
		...
		assert_eq!(XStaking::this_session_reward(), (INITIAL_REWARD / 4) as u128);

		XStaking::mint(&who, 175_000_000_000_000_u128);
		assert_eq!(XStaking::this_session_reward(), (INITIAL_REWARD / 4) as u128);

		XStaking::mint(&who, (FIXED_TOTAL / 8 - 175_000_000_000_000) as u128);
		assert_eq!(XStaking::this_session_reward(), (INITIAL_REWARD / 8) as u128);
	});
}
